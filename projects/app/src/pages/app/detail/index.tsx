...
const AppDetail = () => {
  ...
  useEffect(() => {
    ...
    if (appDetail._id) {
      ...
      if (currentTab === TabEnum.publish && !canAppTypePublish(appDetail.type)) {
        route2Tab(TabEnum.appEdit);
        return;
      }

      if (!appDetail.permission.hasWritePer && canAppTypePublish(appDetail.type)) {
        route2Tab(TabEnum.publish);
      }
    }
  }, [appDetail._id, appDetail.permission.hasWritePer, appDetail.type, currentTab, route2Tab]);
